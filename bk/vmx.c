...
MODULE_INFO(version, "kvm-kmod-3.10.1");
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,31)
static int __read_mostly emulate_invalid_guest_state = true;
#else
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,31)
static int __read_mostly enable_apicv = 1;
#else
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,31)
static int __read_mostly enable_shadow_vmcs = 1;
#else
...
/*jack code*/
//static void vmx_get_cr3(struct kvm_vcpu *vcpu, void *cr3_addr);
/*jack code*/
...
#if defined(CONFIG_KEXEC) && LINUX_VERSION_CODE >= KERNEL_VERSION(3,8,0)
/*
...
{
	return target_tsc - kvm_native_read_tsc();
}
...
static int hardware_enable(void *garbage)
{
	...
	kvm_native_store_gdt(&__get_cpu_var(host_gdt));
...
/*declare function*/
static int getHandleTable(struct kvm_vcpu *vcpu,u32 *handle_table0);
static int getProcessByHandle(struct kvm_vcpu *vcpu,u32 processhandle,u32 handle_table,u32 *objectbody);
/*is process in sec list
 * vcpu(in):pointer to current kvm_vcpu 
 * processhandle(in): target process handle
 * isOK(out):staus identify weather a process is in sec list,1 in 0 out
 *
 * */
static int handle_to_pid(struct kvm_vcpu *vcpu,u32 processhandle,u32 *pname,u32 *processObj){
	u32 handle_table;
	u32 processID;
	u32 objectadd;
	...
	if(processObj!=NULL&&objectadd!=0)
		*processObj=objectadd;
	...
	if(processID==0||processID==4)
		kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt, objectadd+0x16c, pname,
				15, NULL);
	else
		if(get_proname_by_path(vcpu,objectadd,pname)==0)
			return 0;
	return processID;
}
static bool process_is_in_sec(struct kvm_vcpu *vcpu,char *pname,u64 *sep){
	if((*sep=(u64)find_se_process_by_pname(&vcpu->kvm->se_pro_list.pro_list,pname))==NULL){
		return 0; //not security process
	}else	
		return 1;//is security process
}
/*is source process a third process*/
static int is_third_process(char *sou_name,ProcessInfo *sep){
	u32 processadd;
	u32 pid;
	/*get current CR3*/
	...
	char *validprocess="explorer.exe";
	char *validprocess1="csrss.exe";
	char *validprocess2="svchost.exe";
	printk("source process:%s----------------------target process:%s\n",sou_name,sep->image_name);
	/*is process self*/
	if(strcmp(sou_name,sep->image_name)==0)
		return 0;
	/*explorer is valid parent process*/
	if(strcmp(sou_name,validprocess)==0||strcmp(sou_name,validprocess1)==0||strcmp(sou_name,validprocess2)==0)
		return 0;
	return 1;
}
/*handle not sec process*/
#define PROCESS_BEHAVIOR_MASK 0xFFFFFFFF
void getop(char *op,unsigned int behav){
	u32 a=behav&PROCESS_BEHAVIOR_MASK;
	switch(a){
		case 1:strcpy(op,"ReadMemory");
		       break;
		case 2:strcpy(op,"WriteMemory");
		       break;
		case 4:strcpy(op,"AttachProcess");
		       break;
		case 8:strcpy(op,"TerminateProcess");
		       break;
		case 16:
		       break;
		case 32:
...
static int getNowTime(char *time_val){
	struct timex txc;
	struct rtc_time tm;
	do_gettimeofday(&(txc.time));
	rtc_time_to_tm(txc.time.tv_sec,&tm);
	sprintf(time_val, "%04d/%02d/%02d-%02d:%02d:%02d",tm.tm_year+1900
			,tm.tm_mon+1
			,tm.tm_mday
			,tm.tm_hour+8
			,tm.tm_min
...
int recordBehavior(struct kvm_vcpu *vcpu,char *source,char *tar,unsigned int behav){
	char operation[20];
	char time_val[20];
	getop(operation,behav);
	getNowTime(time_val);
	printk("%s %s %s %s\n",source,operation,tar,time_val);
	LogInfo *temp=vcpu->kvm->loginfo;
	int index=vcpu->kvm->logindex;
	if(index<50){
		strcpy(temp[index].sou_pro,source);
		strcpy(temp[index].tar_pro,tar);
		strcpy(temp[index].operations,operation);
		strcpy(temp[index].a_time,time_val);
		vcpu->kvm->logindex++;
	}
	return 1;
}
...
static int handle_exit_by_vmcall(struct kvm_vcpu *vcpu,int nr,u32 processHandle){
	...
	if(!process_is_in_sec(vcpu,tar_name,&sepp))
	{
		goto end_handle;
	}
	if(!sepp){
	...
	ProcessInfo *sep=(ProcessInfo *)((u64 *)sepp);
	//goto end;
	switch (nr){
		/*attach process*/
		case 96:
			if(is_third_process(sou_name,sep)){
				recordBehavior(vcpu,sou_name,tar_name,4);
				kvm_register_write(vcpu,VCPU_REGS_RAX,0xC0000022);
				vmcs_writel(GUEST_RIP,vcpu->kvm->sysenter_eip.oldeip+0x12a);
				return 1;
			}
		...
		case 277:
			if(is_third_process(sou_name,sep)){
				recordBehavior(vcpu,sou_name,tar_name,1);
				kvm_register_write(vcpu,VCPU_REGS_RAX,0xC0000022);
				vmcs_writel(GUEST_RIP,vcpu->kvm->sysenter_eip.oldeip+0x12a);
				return 1;			
			}
		...
		case 370:
			if(is_third_process(sou_name,sep)){
				recordBehavior(vcpu,sou_name,tar_name,8);
				kvm_register_write(vcpu,VCPU_REGS_RAX,0xC0000022);
				vmcs_writel(GUEST_RIP,vcpu->kvm->sysenter_eip.oldeip+0x12a);
				return 1;
			}
		...
		case 399:
			if(is_third_process(sou_name,sep)){
				recordBehavior(vcpu,sou_name,tar_name,2);
				kvm_register_write(vcpu,VCPU_REGS_RAX,0xC0000022);
				vmcs_writel(GUEST_RIP,vcpu->kvm->sysenter_eip.oldeip+0x12a);
				return 1;
			}
...
static int getFileNameByAttributes(struct kvm_vcpu *vcpu,ObjectAttributes *attributes,char *fname){
	int i=0;
	int a=0;
	UNICODE_STRING pname;
	u16 *processName_w=(u16 *)kmalloc(512,GFP_KERNEL);
	char proname_s[256];
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt, attributes->punicode_name, &pname,sizeof(UNICODE_STRING), NULL)) {
		printk("get unicode name fail!\n");
		return 0;
	}
	if(kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,pname.buffer, processName_w,pname.length, NULL)){
		printk("pname.buffer:%08x--------pname.length:%d\n",pname.buffer,pname.length);
		printk("get name buffer fail!\n");                
		return 0;
	}
	for(i=pname.length/2;i>0;i--){
		if(processName_w[i]=='\\'){
			a=i;
			break;
		}
	}
	for(i=0;i<(pname.length/2-a);i++){
		proname_s[i]=processName_w[a+i+1];
	}
	proname_s[i-1]=0;
	memcpy(fname,proname_s,256);
	kfree(processName_w);
	return 1;
}
static int handle_exit_pro_file(struct kvm_vcpu *vcpu,int nr,u32 pobjectattritebus){
	...
	if(ret==0){
		printk("get name error\n");
		goto end;
	}
	if(strcmp(fname,"ntkrnlpa.exe")==0)
		printk("file name:%s\n",fname);
	switch(nr){
		case 66:
			break;
...
static int handle_exception(struct kvm_vcpu *vcpu)
{
	...
	switch (ex_no) {
		case DB_VECTOR:
			/*jack begin*/
			if(get_curr_apc_processName(vcpu,&curr_processadd,pname,&apcadd,&belongpro)==0)
				goto system_handle_db;
			printk("debug process name:%s\n",pname);
			if(find_se_process_by_pname(&vcpu->kvm->se_pro_list.pro_list,pname)!=NULL){
...
static int handle_ept_violation(struct kvm_vcpu *vcpu)
{
	...
	if(gva_eip==0xffffffff){
		...
		u32 nr_add;
		printk("sys call number:%d\n",nr);
		nr_add = *(u32 *)(vcpu->kvm->vm_info.ssdt+nr*4);
		printk("nr_add:%llx\n",nr_add);
		kvm_register_write(vcpu,VCPU_REGS_RIP,nr_add);
		return 1;
	}
...
static int getPageFromNonsList(struct kvm_vcpu *vcpu,u64 KpcrBase,NonPageAddress *nonpage){
	u32 knode_add;
	...
	kvm_read_guest_virt(&vcpu->arch.emulate_ctxt, nonPageSlistHead.next,&nextPage,
			sizeof(unsigned int), NULL);
	kvm_read_guest_virt(&vcpu->arch.emulate_ctxt, nextPage,&thirdPage,
			sizeof(unsigned int), NULL);
	printk("first nonpage address:0x%08x\n",nonPageSlistHead.next);
	printk("second nonpage address:0x%08x\n",nextPage);
	printk("third nonpage address:0x%08x\n",thirdPage);
	nonpage->first_non_page=nonPageSlistHead.next;
	nonpage->second_non_page=nextPage;
	nonPageSlistHead.next=thirdPage;
	nonPageSlistHead.depth-=2;
	if(kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,knode_add+0x8,&nonPageSlistHead,sizeof(nonPageSlistHead), NULL)){
...
/*get page from knode end*/
/*get idt address*/

static int getIDT(u32 *idtbase){
	*idtbase=vmcs_readl(GUEST_IDTR_BASE);
	if(*idtbase!=NULL)
		return 1;
	else
		return 0;
...
u32 servicetablebase;
u32 newservicetable;
/*static int createNewSsdttest(struct kvm_vcpu *vcpu){
  if(kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,servicetablebase,newservicetable,4,NULL)){
  printk("write ssdt structure error!\n");
  return 0;
  }
  return 1;
  }*/
/*get handle table of current process
 *vcpu(in):pointer to current kvm-vcpu structure
 * hanlde_table0(out):address of target handletable
 * */
static int getHandleTable(struct kvm_vcpu *vcpu,u32 *handle_table0){
	...
	u32 curr_process;
	u32 handle_table;
	vmx_get_segment(vcpu,&seg_fs,VCPU_SREG_FS);
	if(seg_fs.base==0x0){
		printk("kpcrbase error!\n");
		return 0;
	}
	kpcrbase=seg_fs.base;
	/*get current kthread*/
	...
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt, curr_process+0xf4, &handle_table,
				4, NULL)) {
		printk("get handle table failed !\n");
		return 0;
	}
	*handle_table0=handle_table;
	return 1;
}
/*resolve handle table*/
static int getProcessByHandle(struct kvm_vcpu *vcpu,u32 processhandle,u32 handle_table,u32 *objectbody){
	u32 tableCode;
	u32 CapturedTable;
	u32 index;
	...
	u32 objectadd;
	char objecttype;
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,handle_table, &tableCode,
				4, NULL)) {
		printk("get  table code failed !\n");
		return 0;
	}
	index=tableCode&0x3;
	CapturedTable=tableCode-index;
	int i=0,j=0,k=0,a=0;
	switch(index){
		case 0:
			TableLevel1=CapturedTable;
			table_entry_add=TableLevel1+(processhandle*2);
			if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,table_entry_add, &table_entry,
						4, NULL)) {
				printk("get  table_entry0 failed !\n");
				return 0;
			}
			/*	printk("table_entry:%08x\n",table_entry);
				if (kvm_read_guest_virt(&vcpu->arch.emulate_ctxt,table_entry, &objectadd,
				4, NULL)) {
				printk("get table_entry0 failed !\n");
				return 0;
				}*/
			...
			break;
		case 1:
			TableLevel2=CapturedTable;
			i=processhandle%(2048);
			processhandle-=i;
			j=processhandle/512;
			if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,TableLevel2+j, &TableLevel1,
						4, NULL)) {
				printk("get  TableLevel1 failed !\n");
				return 0;
			}
			table_entry_add=TableLevel1+(i*2);
			if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,table_entry_add, &table_entry,
						4, NULL)) {
				printk("get TableLevel1 failed !\n");
				return 0;
		...
		case 2:
			...
			k-=j;
			k/=1024;
			if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,TableLevel3+k, &TableLevel2,
						4, NULL)) {
				printk("get  TableLevel3 failed !\n");
				return 0;
			}		
			if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,TableLevel2+j, &TableLevel1,
						4, NULL)) {
	...
	if(!a){
		printk("get object error!\n");
		return 0;
	}
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,(table_entry&(~0x7))+0xc, &objecttype,
				1, NULL)) {
		printk("get  object type failed !\n");
		return 0;
	}
	if((u32)objecttype!=7){
		printk("not process object!\n");
		return 0;
	}
	*objectbody=(table_entry&(~0x7))+0x18;
	return 1;	
}

/*get ssdt address*/
static int getSSDT(struct kvm_vcpu *vcpu,u64 KpcrBase,ServiceDescriptorTableEntry_g *ssdt){
	u32 curr_thread;
	u32 servicetable;
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt, KpcrBase+0x124, &curr_thread,
				4, NULL)) {
	...
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt, curr_thread+0xbc, &servicetable,
				4, NULL)) {
		printk("get service table failed !\n");
		return 0;
	}
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt, servicetable, ssdt,
				sizeof(ServiceDescriptorTableEntry_g), NULL)) {
		printk("get service table failed !\n");
		return 0;
	}
	servicetablebase=servicetable;
	printk("ssdt->base:%p\n",vcpu->kvm->service_table->ServiceTableBase);
	//vcpu->kvm->service_table=ssdt;
	return 1;
}
/*get ssdt address end*/
#ifndef PROCESS_SECURITY
/*ssdt*/
...
#endif
/*create new ssdt*/
static int createNewSsdt(struct kvm_vcpu *vcpu,ServiceDescriptorTableEntry_g *ssdt,u32 newSsdtBase){
	u64 *ssdt_con=&vcpu->kvm->vm_info.ssdt;
	unsigned int r=0;
	ServiceDescriptorTableEntry_g newSsdt;
	newSsdt.ServiceTableBase=newSsdtBase+sizeof(ServiceDescriptorTableEntry_g)*4;
	newSsdt.ServiceCounterTableBase=ssdt->ServiceCounterTableBase;
	...
	if(kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,newSsdtBase+0x20,&newSsdt,sizeof(ServiceDescriptorTableEntry_g),NULL)){
		printk("write ssdt structure error!\n");
		return 0;
	}
	/*pre alloc memory for shadow ssdt*/
	offset+=sizeof(ServiceDescriptorTableEntry_g)*3+0x10;
	vcpu->kvm->ssdtbase=newSsdtBase+offset;
	*ssdt_con=(u64)vmx_alloc(1604);
	printk("*ssdt_con:%p\n",*ssdt_con);
	printk("vcpu->kvm->vm_info.ssdt:%llx\n",vcpu->kvm->vm_info.ssdt);
	printk("ssdt->ServiceTableBase:%llx\n",ssdt->ServiceTableBase);
	if(kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdt->ServiceTableBase,(void *)*ssdt_con,1604, NULL)){
		printk("read ssdt error!\n");
		vmx_free(ssdt_con);
		return 0;
	}else{
		printk("read ssdt structure OK!\n");
	}
	printk("newssdtcon:%llx\n",newSsdtBase+offset);
	if(r=kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,newSsdtBase+offset,(void *)*ssdt_con,1604, NULL)){
	...
	if(r=kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,newSsdtBase+offset+CREATE_FILE*4,&data,4, NULL)){
		printk("clear CREATE_FILE error,r:%d\n",r);
		return 0;
	}else{
		printk("clear CREATE_FILE OK!\n");
	}
	if(r=kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,newSsdtBase+offset+OPEN_FILE*4,&data,4, NULL)){
		printk("clear OPEN_FILE error,r:%d\n",r);
		return 0;
	}else{
		printk("clear OPEN_FILE OK!\n");
	}
	if(r=kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,newSsdtBase+offset+CREATE_PROCESS*4,&data,4, NULL)){
		printk("clear CREATE_PROCESS error,r:%d\n",r);
		return 0;
	}else{
		printk("clear CREATE_PROCESS OK!\n");
	}
...
unsigned int  checkShadowSSDT(struct kvm_vcpu *vcpu,u32 oril_ssdtbase){
	ServiceDescriptorTableEntry_g temp;
	...
	printk("temp.ServiceTableBase:%08x\n",temp.ServiceTableBase);
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt, temp.ServiceTableBase, &firstadd, 4, NULL)) {
		printk("get service table failed 2!\n");
		return 0;
	}
	return firstadd;
}
/*shadow ssdt store in entire page
 *KeServiceDescriptorTable structure
 *ssdt content
 * */
static int createNewShadowSSDT(struct kvm_vcpu *vcpu,u32 oril_ssdtbase,u32 newssdtbase){
	...
	newShadowSsdt.ServiceTableBase=newssdtbase;
	newShadowSsdt.ServiceCounterTableBase=oril_ssdt.ServiceCounterTableBase;
	newShadowSsdt.NumberOfServices=oril_ssdt.NumberOfServices;
	newShadowSsdt.ParamTableBase=oril_ssdt.ParamTableBase;
	printk("oril_ssdt.ServiceTableBase:%08x\n",oril_ssdt.ServiceTableBase);
	printk("newShadowSsdt.NumberOfServices:%08x\n",newShadowSsdt.NumberOfServices);
	u32 shadow_add=vcpu->kvm->nonpage.first_non_page+sizeof(ServiceDescriptorTableEntry_g)*3;
	if(kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,shadow_add,&newShadowSsdt,sizeof(ServiceDescriptorTableEntry_g),NULL)){
		printk("write ssdt structure error!\n");
		return 0;
	}
	u32 *tempssdt=(u32 *)kmalloc(newShadowSsdt.NumberOfServices*4,GFP_KERNEL);
	if(tempssdt==NULL){
		printk("malloc memory error\n");
		return 0;
	}
	if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,oril_ssdt.ServiceTableBase, tempssdt, newShadowSsdt.NumberOfServices*4, NULL)) {
		printk("get service table failed !\n");
		return 0;
	}
	//	printk("tempssdt:%08x\n",tempssdt[0]);
...
static int createNotifyRoutine(struct kvm_vcpu *vcpu,u32 pagebase){
	 ...
	 **
	 ** */
	char code0[] = {0x55, 0x8B, 0xEC, 0x60, 0x8B, 0x5D, 0x10, 0x8B, 0x4D, 0x0c, 0xB8, 0x26, 0x00, 0x00, 0x00, 0x0f, 0x01, 0xc1, 0x61,0x5d, 0xC2, 0x0c, 0x00};
...
/*create new IDT*/
static int createNewIdt(struct kvm_vcpu *vcpu,u32 idtBase,u32 newIdtBase){
	u64 *idt_con=&vcpu->kvm->vm_info.idt;
	*idt_con=vmx_alloc(2048);
	unsigned int r=0;
	unsigned int offset=0;
	if(kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,idtBase,*idt_con,2048, NULL)){
		printk("read idt error!\n");
		vmx_free(idt_con);
		return 0;
	}else{
		printk("read idt OK!\n");
	}
	printk("new idt base:0x%08x\n",newIdtBase+offset);
	if(r=kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,newIdtBase,*idt_con,2048, NULL)){
		printk("write IDT error,r:%d\n",r);
		return 0;
	}else{
		printk("write idt OK!\n");
	}
...
/*create new IDT end*/
/*create jmp fun*/
static int createJmpFun(struct kvm_vcpu *vcpu,u32 fastcallAdd,unsigned int newBase,unsigned int ssdt,unsigned int offset_fun,u32 *newfun){
	unsigned int r=0,offset=0;
	u32 new_fastcalladd;
	/*
	 *code0 175 bytes of original function + push return 	
	 *jmpcode1 20 bytes if target is ssdt or shadow ssdt
	...
	u32 guifun_wadd=0;
	u32 fastcall_w8f=0;
	u32 fastcall_w170_1=0;
	u32 fastcall_w170_2=0;
	if(r=kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,fastcallAdd+171,&view_ss_add,4, NULL)){
		printk("write memory_eip error,r:%d\n",r);
		return 0;
	}
	KiBBTUnexpectedRangeadd=fastcallAdd+175+view_ss_add;
	if(r=kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,KiBBTUnexpectedRangeadd+8,&guifun_vadd,4, NULL)){
		printk("write memory_eip error,r:%d\n",r);
		return 0;
	}
	guifun_radd=KiBBTUnexpectedRangeadd+12+guifun_vadd;
	printk("guifun_radd:%08x\n",guifun_radd);
	guifun_wadd=guifun_radd-(newBase+offset+12);
	fastcall_w8f=newBase+76+0x8f-(newBase+offset+30);
	fastcall_w170_1=fastcallAdd+0x170-(newBase+offset+66);
	fastcall_w170_2=fastcallAdd+0x170-(newBase+offset+76);
	//printk("KiBBTUnexpectedRange:%08x\n",fastcallAdd+175+view_ss_add);
	u32 unexpect_wadd=vcpu->kvm->nonpage.first_non_page+0x30;
	if(r=kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,fastcallAdd+175+view_ss_add,KiBBTUnexpectedRange,76, NULL)){
		printk("write memory_eip error,r:%d\n",r);
		return 0;
	}
	*(u32 *)&KiBBTUnexpectedRange[8]=guifun_wadd;
	*(u32 *)&KiBBTUnexpectedRange[26]=fastcall_w8f;
	*(u32 *)&KiBBTUnexpectedRange[32]=unexpect_wadd;
	*(u32 *)&KiBBTUnexpectedRange[62]=fastcall_w170_1;
	*(u32 *)&KiBBTUnexpectedRange[72]=fastcall_w170_2;
	/*write KiBBTUnexpectedRange function*/
	if(r=kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,newBase+offset,KiBBTUnexpectedRange,76, NULL)){
		printk("copy jmp code2  error:%d\n",r);
		return 0;
	}

	printk("new KiBBTUnexpectedRange:%08x\n",newBase);
	offset+=76;
	/*construct kifastcallentry function*/
	if(r=kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,fastcallAdd,code0,175, NULL)){
		printk("write memory_eip error,r:%d\n",r);
		return 0;
	}
	*(u32 *)&code0[171]=newBase-(newBase+offset+175);
	code0[153]=0xe9;
	code0[158]=0x90;
	code0[175]=0x68;
	code0[180]=0xc3;
	*(u32 *)&code0[176]=fastcallAdd+175;
	if(r=kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,newBase+offset,code0,181, NULL)){
		printk("copy jmp code2  error:%d\n",r);
		return 0;
	}
...
static int deploySecuritySystem(struct kvm_vcpu *vcpu,u32 sysenter_tip,u32 *newfun,u32 *idtbase){
	...
	u32 newBase;
	u32 idtBase;
	NonPageAddress *nonpage;
	u32 keservicetableadd;
	unsigned int r=0;
	unsigned int offset=0;
	vcpu->kvm->service_table=vmx_alloc(sizeof(ServiceDescriptorTableEntry_g));
	nonpage=&vcpu->kvm->nonpage;
	if(getKpcrBase(vcpu,&kpcrbase)==0)
		return 0;
	if(getPageFromNonsList(vcpu,kpcrbase,nonpage)==0)
		return 0;
	newBase=nonpage->first_non_page;
	printk("newBase:%08x\n",newBase);
	...
	if((r=createNewSsdt(vcpu,vcpu->kvm->service_table,newBase))==0)
		return 0;
	offset+=r;
	printk("nonpage.second_non_page:%08x\n",nonpage->second_non_page);
	if((r=createJmpFun(vcpu,sysenter_tip,newBase+offset,newBase,0x99,newfun))==0){
		return 0;
	}
	offset+=r;
	createNotifyRoutine(vcpu,newBase+offset);
	vcpu->kvm->notifybase=newBase+offset;
	return 1;
}
/*deploy system end*/

/*free vm info when deploye failed*/
static void vm_info_free_infail(struct kvm_vcpu *vcpu){
	if(vcpu->kvm->vm_info.ssdt!=NULL){
		vmx_free(vcpu->kvm->vm_info.ssdt);
		vcpu->kvm->vm_info.ssdt=NULL;
	}
	if(vcpu->kvm->vm_info.idt!=NULL){
		vmx_free(vcpu->kvm->vm_info.idt);
		vcpu->kvm->vm_info.idt=NULL;
	}
...
static void __noclone vmx_vcpu_run(struct kvm_vcpu *vcpu)
{
	...
	spinlock_t *p_lock=&vcpu->kvm->alloc_lock;
...
//	}
	...
	if(servicetablebase!=0&&isflag==0){
		...
		if(shssdt!=0){//do something
			printk("shssdt:%08x\n",shssdt);
			if(createNewShadowSSDT(vcpu,servicetablebase+0x50,vcpu->kvm->nonpage.second_non_page)){
				shssdtdone=1;
			}
			isflag=1;
		}
	...
	if((vcpu->kvm->is_alloc==1&&(vm_exec&CPU_BASED_USE_MSR_BITMAPS))){
...
static int __init vmx_init(void)
{
	...
	if (enable_ept) {
		...
		kvm_disable_tdp();
	printk(KERN_ERR "******************************\n");
	printk(KERN_ERR "KVM_JACK Successfully loaded!\n");
	printk(KERN_ERR "******************************\n");
...
#endif
	printk("******************************\n");
	printk("KVM_JACK Successfully unloaded!\n");
	printk("******************************\n");
