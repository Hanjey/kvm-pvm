...
static int kvm_guest_time_update(struct kvm_vcpu *v)
{
        ...
        struct kvm_pvclock_vcpu_time_info guest_hv_clock;
        u8 pvclock_flags;
        ...
        case MSR_KVM_SYSTEM_TIME_NEW:
                case MSR_KVM_SYSTEM_TIME: {
                        ...
                        if (kvm_gfn_to_hva_cache_init(vcpu->kvm,
                                                      ...
                                                      sizeof(struct kvm_pvclock_vcpu_time_info)))
                        vcpu->arch.pv_time_enabled = false;
                        else
...
/*jack code */
static int kvm_get_spte32(struct kvm_vcpu *vcpu,gva_t addr,u64 *spte,gpa_t cr3_value){
        unsigned int offset_1=addr>>22;
        unsigned int offset_2=addr>>12&0x3ff;
        unsigned long spte1;
        kvm_read_guest(vcpu->kvm,cr3_value+offset_1*4,&spte1,4);
        unsigned long walk_2=spte1&0xfffff000;
        kvm_read_guest(vcpu->kvm,walk_2+offset_2*4,spte,4);
        printk("0x%08x----spte-value----0x%08x\n",addr,spte);
        return 1;
}
/*
static int kvm_get_spte64(gva_t addr,unsigned long *spte,gpa_t cr3_value){
...
static u64 kvm_get_spte_pae(struct kvm_vcpu *vcpu,gva_t addr,u64 *spte,gpa_t cr3_value){
        ...
        unsigned int offset_3=addr>>12 & 0x1ff;
        u64 pdspte;
        u64 ptspte;
        cr3_value |= offset_1*8;
        kvm_read_guest(vcpu->kvm,cr3_value,&pdspte,8);
        if(pdspte&0x1==0)
        goto errorpage;
        u64  walk_2=pdspte&(~0xfff)|(offset_2*8);
        if(pdspte&0x80==1)
        goto bigpage;
        kvm_read_guest(vcpu->kvm,walk_2,&ptspte,8);
        if((ptspte)&0x1==0)
        goto errorpage;
        if(ptspte&0x80==1)
        goto bigpage;
        u64 walk_3=ptspte&(~0xfff)|(offset_3*8);
        kvm_read_guest(vcpu->kvm,walk_3,spte,8);
        //      u64 sptel=*spte;
        //	sptel &= ~0x2;
        //	kvm_write_guest(vcpu->kvm,walk_3,&sptel,8);
        //	kvm_read_guest(vcpu->kvm,walk_3,spte,8);
        printk("walk_3:%llx\n",walk_3);
        return walk_3;
        bigpage:
        printk("it is a big page!\n");
        return 0;
        errorpage:
        printk("it is a error page!\n");
        return 0;
...
unsigned int kvm_get_guest_last_spte(struct kvm_vcpu *vcpu,gva_t addr,u64 *last_spte)
{
        ...
        if (!is_paging(vcpu)) {
                ...
                //		printk("in pae mod!\n");
                return kvm_get_spte_pae(vcpu,addr,last_spte,cr3_value);
        }else{
                printk("in 32 mod!\n");
                return kvm_get_spte32(vcpu,addr,last_spte,cr3_value);
        }
        return 1;
}
...
void viewSlotInfo(struct kvm_memory_slot *slot){
        printk("slot----------------------------info\n");
        printk("slot base_gfn:0x%llx\n",slot->base_gfn);
        printk("slot npages:0x%08x\n",slot->npages);
        printk("slot userspace_addr:0x%08x\n",slot->userspace_addr);
        printk("slot flags:0x%08x\n",slot->flags);
        printk("slot id:%d\n",slot->id);
        if(slot->dirty_bitmap)
        printk("slot bitmap:0x%08x\n",slot->dirty_bitmap);
        else
        printk("slot bitmap is NULL\n");
        printk("slot---------info--------------end\n");
}

void foreachSlotBitmap(struct kvm_memslots *slots){
        struct kvm_memory_slot *memslot;
...
void viewSSDT(struct kvm_vcpu *vcpu,gva_t ssdt_base){
        int i=0;
        unsigned int value;
        for(i=0;i<401;i++){
...
int kvm_alloc_vm_page1(gva_t ssdt_base,gva_t nonpagedpoolstart,struct kvm_vcpu *vcpu,u32 mmpfn){
        ...
        struct kvm_memory_slot *main_slot;
        gpa_t nonpagepool_gpa=kvm_mmu_gva_to_gpa_system(vcpu,nonpagedpoolstart,&exception);
        printk("nonpagepool_gpa:0x%llx\n",nonpagepool_gpa);
        temp_gfn=nonpagepool_gpa>>12;
        printk("temp_gfn:0x%llx\n",temp_gfn);
        main_slot=gfn_to_memslot(vcpu->kvm,temp_gfn);
        if(main_slot==NULL){
                printk("could not find main slot!\n");
                return 0;
        }
        viewSlotInfo(main_slot);
        int i=0,r=0,d=0;
        unsigned long rel_gfn=temp_gfn-main_slot->base_gfn;
        unsigned int data=0x1a2b3c4d;
        if(main_slot->dirty_bitmap){
                unsigned long  *p;
        ...
        printk("i=%d\n",i);
        if(d<800)return -1;
        new_add=(nonpagedpoolstart&(~0xfff)) + i*4096;
        u64 new_spte;
        kvm_get_guest_last_spte(vcpu,new_add,&new_spte);
        if(new_spte&0x1==0){
...
printk("value=%08x\n",value);
if(r=kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,new_add,&data,4, NULL)){
        printk("write error:r=%d\n",r);
        return -1;
}
printk("write OK!\n");
kvm_read_guest_virt(&vcpu->arch.emulate_ctxt,new_add,&value,4, NULL);
...
EXPORT_SYMBOL_GPL(kvm_alloc_vm_page1);
/*jack code*/
                                ...
                                struct x86_exception *exception)
{
        ...
        while (bytes) {
                ...
                if (gpa == UNMAPPED_GVA){
                        printk("error GPA :%016x-------corrgva:%016x\n",gpa,addr);
                        return X86EMUL_PROPAGATE_FAULT;
                }
...
static void emulator_get_gdt(struct x86_emulate_ctxt *ctxt, struct kvm_desc_ptr *dt)
{
        kvm_x86_ops->get_gdt(emul_to_vcpu(ctxt), dt);
}

static void emulator_get_idt(struct x86_emulate_ctxt *ctxt, struct kvm_desc_ptr *dt)
{
        kvm_x86_ops->get_idt(emul_to_vcpu(ctxt), dt);
}

static void emulator_set_gdt(struct x86_emulate_ctxt *ctxt, struct kvm_desc_ptr *dt)
{
        kvm_x86_ops->set_gdt(emul_to_vcpu(ctxt), dt);
}

static void emulator_set_idt(struct x86_emulate_ctxt *ctxt, struct kvm_desc_ptr *dt)
{
...
static bool emulator_get_segment(struct x86_emulate_ctxt *ctxt, u16 *selector,
                                 struct kvm_desc_struct *desc, u32 *base3,
                                 int seg)
{
...
{
        return kvm___this_cpu_read(current_vcpu) != NULL;
}
...
static int kvm_is_user_mode(void)
{
        ...
        if (kvm___this_cpu_read(current_vcpu))
        user_mode = kvm_x86_ops->get_cpl(kvm___this_cpu_read(current_vcpu));
...
static unsigned long kvm_get_guest_ip(void)
{
        ...
        if (kvm___this_cpu_read(current_vcpu))
        ip = kvm_rip_read(kvm___this_cpu_read(current_vcpu));
...
{
        kvm___this_cpu_write(current_vcpu, vcpu);
}
EXPORT_SYMBOL_GPL(kvm_before_handle_nmi);

void kvm_after_handle_nmi(struct kvm_vcpu *vcpu)
{
        kvm___this_cpu_write(current_vcpu, NULL);
}
...
{
        u64 mask;
        int maxphyaddr = kvm_x86_phys_bits;
...
static int foreach_process_list(struct list_head *head){
        ...
        while(p!=head){
                ...
                printk("[%2d] process id: [%4d]  process name: [%20s]  \n",i,temp->u1.pro_id,temp->image_name);
                else
...
static int query_all_process(struct kvm_vcpu *vcpu, unsigned int pro_list_address){
        printk("query all process\n");
        struct list_head *head,*p;
        ProcessInfo *temp;
        head=&vcpu->kvm->true_process_list.pro_list;
        p=head->next;
        while(p!=head){
                temp=(ProcessInfo *)p;
                if(kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt, pro_list_address,&temp->u1.pro_id,4, NULL)){
...
static int add_se_process(struct kvm_vcpu *vcpu,unsigned int pa){
        ...
        ProcessInfo *sepro;
        ProcessInfo *temp;
        sepro=(ProcessInfo *)kmalloc(sizeof(ProcessInfo),GFP_KERNEL);
...
static int remove_se_process(struct kvm_vcpu *vcpu,unsigned int pa){
        ...
        vcpu->kvm->se_pro_list.u1.pro_count--;
        printk("del security  process :%s------total count:%d\n",pname,vcpu->kvm->se_pro_list.u1.pro_count);
        foreach_process_list(&vcpu->kvm->se_pro_list.pro_list);
        return 1;
}
...
static int del_process_in_list(struct kvm_vcpu *vcpu,u32 processID){
        struct list_head *p;
        p=find_se_process_by_pid(&vcpu->kvm->true_process_list.pro_list,processID);
        del_list(&vcpu->kvm->true_process_list.pro_list,p);
...
static int add_process_to_list(struct kvm_vcpu *vcpu,u32 next_process,u32 targetpid){
        ProcessInfo *tempNode;
        u32 punicode_name;		
        //	memset(proname_s,0,30);
        tempNode=(ProcessInfo *)kmalloc(sizeof(ProcessInfo),GFP_KERNEL);
        tempNode->u1.pro_id=targetpid;
        ...
        insert_node:
        printk("insert process:%d %s\n",targetpid,tempNode->image_name);
        insert_list(&vcpu->kvm->true_process_list.pro_list,&tempNode->pro_list);
        vcpu->kvm->true_process_list.u1.pro_count++;
        return 1;
}
...
/*tap level*/
static int brosetable1(struct kvm_vcpu *vcpu,u32 tableadd,u32 targetpid);
static int brosetable2(struct kvm_vcpu *vcpu,u32 tableadd,u32 targetpid);
static int brosetable3(struct kvm_vcpu *vcpu,u32 tableadd,u32 targetpid);
...
static int brosetable3(struct kvm_vcpu *vcpu,u32 tableadd,u32 targetpid){
        ...
        while(tableadd!=0&&count<1024){
                ...
                if(table2add!=0)
                brosetable2(vcpu,table2add,targetpid);
                tableadd+=4;
                count++;
        }
        return 1;
...
static int brosetable2(struct kvm_vcpu *vcpu,u32 tableadd,u32 targetpid){
        ...
        while(tableadd!=0&&count<1024){
                ...
                if(table3add!=0)
                brosetable1(vcpu,table3add,targetpid);
                tableadd+=4;
                count++;
        }
        return 1;
...
static int brosetable1(struct kvm_vcpu *vcpu,u32 tableadd,u32 targetpid){
        ...
        do{
                ...
                if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,objectadd+0xb4, &processID,4, NULL)) {
                        printk("get  process ID failed!\n");
                        return 0;
                }
                if(vcpu->kvm->gloflag==0){
                        if(add_process_to_list(vcpu,objectadd,processID)==0){
                                printk("add process %d failed!\n",processID);
                                break;
                        }
                        tableadd+=8;
                        continue;
...
#define JACK_OP_CODE            0x20
#define KVM_ENABLE_SECURITY     0x21
#define KVM_DISABLE_SECURITY    0x22
#define POLICY_OP_CODE          0x23
#define PROCDESS_QUERY_CODE     0x24
#define PROCESS_QUERY_ALL       0x25
#define PROCESS_LIFE            0x26
#define GET_CALLBACK_BASE       0x27
#define GET_LOGINFO             0x28
/*sub op code by jack*/
...
static int get_process_list_by_handletable(struct kvm_vcpu *vcpu,u32 targetpid){
        u32 pspcidtable;
        u32 index;
        u32 tableCode;
        u32 tableadd;
        if(!getPspCidTable(vcpu,vcpu->kvm->kpcrbase,&pspcidtable)){
                printk("get pspcidtable failed!\n");
                return 0;
        }
        if (kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,pspcidtable, &tableCode,4, NULL)) {
        ...
        switch(index){
                case 0:brosetable1(vcpu,tableadd,targetpid);
                break;
                case 1:brosetable2(vcpu,tableadd,targetpid);
                break;
                case 2:brosetable3(vcpu,tableadd,targetpid);
                break;
                default:
                return 0;
                break;
        ...
        if(vcpu->kvm->gloflag==0)
        foreach_process_list(&vcpu->kvm->true_process_list.pro_list);
        return 1;
}

static int processlife(struct kvm_vcpu *vcpu,u32 processID,u32 createFlag){
        //printk("createFlag:%d\n",createFlag);

        if(createFlag){
                if(vcpu->kvm->gloflag==0){
                        printk("first time walk entire cidtable\n");
                        get_process_list_by_handletable(vcpu,processID);
                        vcpu->kvm->gloflag=1;
                }else{
                        get_process_list_by_handletable(vcpu,processID);
                ...
                /*first find process in trul list,then remove it*/
                del_process_in_list(vcpu,processID);
                printk("terminate process:%d\n",processID);
        }
        return 0;
}
static int getLogInfo(struct kvm_vcpu *vcpu,u32 address){
        printk("get Log info\n");
        struct list_head *head,*p;
        LogInfoT temp_logt;
        int index=vcpu->kvm->logindex;
        if(kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,address, &temp_logt,sizeof(LogInfoT), NULL)) {
                 printk("fail to read LogInfoT !\n"); 
                return 0;
        }
        if(kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt, temp_logt.logInfo,vcpu->kvm->loginfo,sizeof(LogInfo)*index, NULL)){
                        printk("fail to write process ID!\n");
                        return 0;
        }
        memset(vcpu->kvm->loginfo,0,sizeof(LogInfo)*50);
        if(kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,address,&index,4, NULL)){
                                        printk("fail to write finish!\n");
                                        return 0;
         }
        vcpu->kvm->logindex=0;
        return 0;
}
static int isValidNR(unsigned long nr){
        if(nr>=0x20&&nr<=0x28){
                return 0;
        }
        return 1;
 ...
 * bit 0 :read memory
 * bit 1 :write memory
 * bit 2 :attach process
 * bit 3 :terminate process
* */
static int oper_policy(struct kvm_vcpu *vcpu,u32 p_num,u32 op){
        u32 nr_add;
        u32 ssdtbase=vcpu->kvm->ssdtbase;
        u32 data=0xFFFFFFFF;
        u32 policy=1<<(p_num-1);
        switch(policy){
               case 1:
               if(op){
                        printk("add READ_MEMORY policy\n");
                        vcpu->kvm->policy|=policy;
                        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdtbase+READ_MEMORY*4,&data,4, NULL);
               }else{
                        printk("remove READ_MEMORY policy\n");
                       vcpu->kvm->policy &= ~policy;
                        nr_add = *(unsigned long *)(vcpu->kvm->vm_info.ssdt+READ_MEMORY);
                        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdtbase+READ_MEMORY*4,&nr_add,4, NULL);
               }
               break;
               case 2:
               if(op){
                       printk("add WRITE_MEMORY policy\n");
                        vcpu->kvm->policy|=policy;      
                        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdtbase+WRITE_MEMORY*4,&data,4, NULL);
               }else{  
                       printk("remove WRITE_MEMORY policy\n");
                        vcpu->kvm->policy &= ~policy;                               
                        nr_add = *(unsigned long *)(vcpu->kvm->vm_info.ssdt+WRITE_MEMORY);
                        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdtbase+WRITE_MEMORY*4,&nr_add,4, NULL);
               }
               break;
               case 4:
                if(op){
                        printk("add DEBUG_ACTIVE_PROCESS policy\n");
                        vcpu->kvm->policy|=policy;    
                        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdtbase+DEBUG_ACTIVE_PROCESS*4,&data,4, NULL);
               }else{  
                       printk("remove DEBUG_ACTIVE_PROCESS policy\n");
                        vcpu->kvm->policy &= ~policy;                      
                        nr_add = *(unsigned long *)(vcpu->kvm->vm_info.ssdt+DEBUG_ACTIVE_PROCESS);
                        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdtbase+DEBUG_ACTIVE_PROCESS*4,&nr_add,4, NULL);
               }
               break;
               case 8:
                if(op){
                        printk("add TERMINAL_PROCESS policy\n");
                        vcpu->kvm->policy|=policy;    
                        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdtbase+TERMINAL_PROCESS*4,&data,4, NULL);
               }else{  
                        printk("remove TERMINAL_PROCESS policy\n");
                        vcpu->kvm->policy &= ~policy;                        
                        nr_add = *(unsigned long *)(vcpu->kvm->vm_info.ssdt+TERMINAL_PROCESS);
                        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,ssdtbase+TERMINAL_PROCESS*4,&nr_add,4, NULL);
               }
               break;
               default:
               break;
...
static int policy_handle(struct kvm_vcpu *vcpu,u32 policy_address,u32 op){
       u32 policy;
        if(op!=KVM_GET){
                kvm_read_guest_virt_system(&vcpu->arch.emulate_ctxt,policy_address,&policy,4, NULL);
       }
        switch(op){
                case KVM_GET:
                kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,policy_address,&vcpu->kvm->policy,4, NULL);
                break;
                case KVM_ADD:
                       oper_policy(vcpu,policy,1);
                break;
                case KVM_REMOVE:
                       oper_policy(vcpu,policy,0);
                break;
                default:
                break;
        }
...
static int getnonpagebase(struct kvm_vcpu *vcpu,u32 address){
        printk("get nonpage address:%08x\n",address);
        kvm_write_guest_virt_system(&vcpu->arch.emulate_ctxt,address,&vcpu->kvm->notifybase,4, NULL);
        return 0;
}
...
int kvm_emulate_hypercall(struct kvm_vcpu *vcpu)
{
        ...
        switch (nr) {
                ...
                ret = 0;
                break;
                /*add or del or get security process*/
                /*a0:ebx a1:ecx*/
                case JACK_OP_CODE:
                ret=jack_handle_op(vcpu,a1,a0);
                break;
                case POLICY_OP_CODE:
                ret=policy_handle(vcpu,a0,a1);
                break;
                /*get callback function address*/
                case GET_CALLBACK_BASE:
                ret=getnonpagebase(vcpu,a0); 
                break;
                /*query process in security*/
                case PROCESS_QUERY_ALL:
                ret=query_all_process(vcpu,a0);
                break;
...
static int vcpu_enter_guest(struct kvm_vcpu *vcpu)
{
        ...
        */
        ...
        kvm_smp_send_reschedule(vcpu->cpu);
...
struct kvm_static_key kvm_no_apic_vcpu __read_mostly;
...
int kvm_arch_vcpu_init(struct kvm_vcpu *vcpu)
{
        ...
        kvm_static_key_slow_inc(&kvm_no_apic_vcpu);
...
void kvm_arch_vcpu_uninit(struct kvm_vcpu *vcpu)
{
        ...
        free_page((unsigned long)vcpu->arch.pio_data);
        if (!irqchip_in_kernel(vcpu->kvm))
        kvm_static_key_slow_dec(&kvm_no_apic_vcpu);
}
