...
int vcpu_load(struct kvm_vcpu *vcpu)
{
	...
	if (unlikely(vcpu->pid != current->pids[PIDTYPE_PID].pid)) {
		...
		struct pid *newpid = kvm_get_task_pid(current, PIDTYPE_PID);
			  ...
			  unsigned long len)
{
	...
	if (kvm_is_error_hva(addr))
		return -EFAULT;
	pagefault_disable();
	r = kvm_read_hva_atomic(data, (void   *)addr + offset, len);
	pagefault_enable();
	if (r)
		return -EFAULT;
	return 0;
				   ...
				   struct kvm_create_device *cd)
{
	...
	ret = kvm_anon_inode_getfd(ops->name, &kvm_device_fops, dev, O_RDWR);
	if (ret < 0) {
		ops->destroy(dev);
		return ret;
	}
...
IF_ANON_INODES_DOES_REFCOUNTS(	kvm_vcpu_fops.owner = module;)
	...
	printk("loaded kvm module (kvm-kmod-3.10.1)\n");
